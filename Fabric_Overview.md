...
It offers a single, standards-based abstraction across messaging, APIs, and storage — making it easier to build client applications and scalable, resilient services that can run anywhere.

Fabric can be run seamlessly on **Windows**, **Linux**, as an **API** (for applications), or **CLI** (for interactive users). When used inside an **IDE**, Fabric integrates directly with **Copilot** for intelligent code completion, schema generation, and rapid developer assistance — enabling faster data-driven development cycles.
